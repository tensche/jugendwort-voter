...
async function solveCaptcha(apiKey) {
  const sitekey = "6Lf5zMAZAAAAAKgTZritepo-zKuRStlrPa06Ts4l";
  const url = "https://www.langenscheidt.com/jugendwort-des-jahres";
  const captchaUrl = `http://2captcha.com/in.php?key=${apiKey}&method=userrecaptcha&googlekey=${sitekey}&pageurl=${url}`;
  ...
  if (response.status === 200) {
    ...
    if (responseText.includes("|")) {
      ...
      const solutionUrl = `http://2captcha.com/res.php?key=${apiKey}&action=get&id=${captchaId}`;
      ...
      while (true) {
        ...
        if (solutionResponse.status === 200) {
          ...
          if (solutionText !== "CAPCHA_NOT_READY") {
            return solutionText.split("|")[1];
          }
        } else {
...
function sanitizeUserAgent(userAgent) {
  // Remove any special characters from the user agent
  return userAgent.replace(/[^a-zA-Z0-9-_.() ]/g, '');
}

function vote(yourWord, code) {
  let userAgents = [
  ...
  const fileContents = fs.readFileSync('user_agents.txt', 'utf8');

  const additionalUserAgents = fileContents.split('\n');

  userAgents = userAgents.concat(additionalUserAgents.map(sanitizeUserAgent));
  ...
  const payload = {
    "jw-word-collect": yourWord,
    "jw-is-word-collect": 1,
    "age": randomNumber,
    "consent": 1,
  ...
  const options = {
    url: "https://www.langenscheidt.com/jw21/voting/vote",
    method: "POST",
    headers: headers,
    form: payload
  };
...
async function main() {
  ...
  const yourWord = "ENTER-YOUR-WORD"

  try {
    const code = await solveCaptcha(apiKey);
    if (code) {
      console.log("Solved the captcha");
      const result = await vote(yourWord, code);
      ...
      console.log("Failed to solve the captcha");
    }
  } catch (error) {
    console.error(error);
  }
